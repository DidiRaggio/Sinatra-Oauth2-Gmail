...
def oauth2full ( client_id, client_secret)
	...
	auth_client_obj = OAuth2::Client.new( client_id.to_s, client_secret.to_s, {:site => 'https://accounts.google.com', :authorize_url => "/o/oauth2/auth", :token_url => "/o/oauth2/token", :ssl => {:verify => false}})
	# STEP 0
	puts "\n\n\n0) insert the email that the client_id and client_secret belong to:\n"
	...
	access_token_obj = auth_client_obj.auth_code.get_token(code, :redirect_uri => redirect_uri)

	# File.open('ato.yaml', 'w') { |fo| fo.puts access_token_obj.to_yaml }
	# STEP 4
	...
	env = Hash[ 'client_id' => client_id, 'client_secret' => client_secret, 'access_token' => access_token_obj.token, 'refresh_token' => access_token_obj.refresh_token, 'username' => email]
